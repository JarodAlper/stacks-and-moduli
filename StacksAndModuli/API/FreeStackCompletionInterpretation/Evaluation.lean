...
public import StacksAndModuli.API.FreeStackCompletionInterpretation.Basic

/-!
# Evaluating free stack completion syntax

This module simultaneously evaluates the object and arrow derivations of the free
stack completion in an arbitrary target stack.
-/
...
/-- Evaluate an object derivation in a target stack. -/
noncomputable def evaluateObj (F : X ⥤ᵇ Y) {t : Pre J X} {S : C}
    (h : IsObj J X t S) : ObjResult F h := by
  refine (IsObj.rec (C := C) (J := J) (X := X)
    ...
    ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ ?_ h).1
  · intro a
...
/-- Evaluate an arrow typing derivation in a target stack. -/
noncomputable def evaluateHom (F : X ⥤ᵇ Y) {phi a b : Pre J X} {S T : C}
    {f : S ⟶ T} (h : IsHom J X phi a b f) : HomResult F h := by
